...
class SyncPipelineTaskGenerator(task_gen.TaskGenerator):
  ...
  def _maybe_generate_task(
    ...
    cache_context = cache_utils.get_cache_context(
        ...
        executor_spec=None,  # TODO(goutham): plumb executor spec.
        input_artifacts=resolved_info.input_artifacts,
    ...
    if node.execution_options.caching_options.enable_cache and cached_outputs:
      logging.info('Eliding node execution, using cached outputs; node uid: %s',
                   node_uid)
